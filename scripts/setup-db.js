...
async function main() {
    ...
    try {
        ...
        await connection.query(`CREATE DATABASE IF NOT EXISTS \`${process.env.DB_NAME || 'jobportal'}\``);
    ...
    `);
        ...
        for (const title of vacancies) {
            const [rows] = await connection.query('SELECT id FROM vacancies WHERE title = ?', [title]);
            if (rows.length === 0) {
                await connection.query('INSERT INTO vacancies (title) VALUES (?)', [title]);
